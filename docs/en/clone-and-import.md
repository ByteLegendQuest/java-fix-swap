...
`git clone https://github.com/ByteLegendQuest/java-fix-swap`

A project directory named `java-fix-swap` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-fix-swap` directory which is created just now.
